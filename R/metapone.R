...
function(pos=NULL, neg=NULL, pa, hmdbCompMZ, pos.adductlist = c("M+H", "M+NH4", "M+Na", "M+ACN+H","M+ACN+Na", "M+2ACN+H", "2M+H", "2M+Na", "2M+ACN+H"), neg.adductlist = c("M-H","M-2H","M-2H+Na","M-2H+K", "M-2H+NH4","M-H2O-H","M-H+Cl", "M+Cl", "M+2Cl"), use.fractional.count=TRUE, match.tol.ppm=5, num.nodes=4, p.threshold=0.05, n.permu=200, fractional.count.power=0.5, max.match.count=10)
{
	...
	for(n in 1:length(dat))
	{
		...
		{
			ion<-this.adductlist[nnn]
			this.kegg<-this.comp.mz[this.comp.mz[,3] %in% ion,]
			m<-mass.match(this.dat[,1], this.kegg[,2], match.tol.ppm=match.tol.ppm)
		
			this.matched<-NULL
			if(sum(m)>0)
			{
				this.matched<-matrix(0, ncol=2+ncol(this.dat)+ncol(this.comp.mz), nrow=1)
	...
	to.return <- new("metapone.result",test.result=rec, mapped.features=rec2)
